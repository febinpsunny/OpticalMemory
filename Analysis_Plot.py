...
spacing=[-0.45,-0.3,-0.15,0.0,0.15,0.3]
colors = ['indigo','lavender','plum','red','orange','azure']
labels=['2D_DRAM','3D_DRAM','PCM_ISSCC','COSMOS_EPCM','COSMOS_OPCM','COSMOS_OPCM_TE']
...
for i in range(0,fileno):
    if (i+1)*len(appName) < len(AverageLatency):
        ax.bar(np.arange(len(appName))+spacing[i],AverageLatency[i*len(appName):(i+1)*len(appName)],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
    else:
        ax.bar(np.arange(len(appName))+spacing[i],AverageLatency[i*len(appName)::],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
...
for i in range(0,fileno):
    if (i+1)*len(appName) < len(AvgBandWidth):
        ax.bar(np.arange(len(appName))+spacing[i],AvgBandWidth[i*len(appName):(i+1)*len(appName)],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
    else:
        ax.bar(np.arange(len(appName))+spacing[i],AvgBandWidth[i*len(appName)::],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
...
for i in range(0,fileno):
    if (i+1)*len(appName) < len(EPB):
        ax.bar(np.arange(len(appName))+spacing[i],EPB[i*len(appName):(i+1)*len(appName)],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
    else:
        ax.bar(np.arange(len(appName))+spacing[i],EPB[i*len(appName)::],width=0.15,align='center',
        color=colors[i],label=labels[i],edgecolor='black',linewidth=1.25)
