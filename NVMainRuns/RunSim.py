...
CycleTime = [1.3e9, 666e6, 400e6, 1e9, 1e9, 400e6, 1e9, 1e9]
...
def SimulationRuns(runCyc="100000"):
    ...
    MaxProgressbarVal = len(os.listdir(ConfigDir))*(len(os.listdir(TraceDir))-8)
    runCycles=runCyc
        ...
        for config in os.listdir(ConfigDir):
            ...
            for trace in os.listdir(TraceDir):
                if '.milc' in trace or '.soplex' in trace or '.lbm' in trace or '.mcf' in trace or '.leslie3d' in trace or '.zeusmp' in trace or '.txt' in trace or '.wrf' in trace:
                    ...
                    bar()
                    cmd=nvmainBinary+" "+ConfigDir+config+" "+TraceDir+trace+" "+runCycles+" >>"+OutDir+config.split('.')[0]+"_"+trace
                    os.system(cmd)
def RWSimRuns(runCyc="0"):
    print("Performing NVMain Read/Write throughput simulations on the main memory configurations")
    os.system("rm "+RWOutputs+"*")
    MaxProgressbarVal = len(os.listdir(ConfigDir))*len(os.listdir(R_W_Traces))
    runCycles=runCyc
    with alive_bar(MaxProgressbarVal) as bar:
        for config in os.listdir(ConfigDir):
            print(config)
            for trace in os.listdir(R_W_Traces):
                bar()
                cmd=nvmainBinary+" "+ConfigDir+config+" "+R_W_Traces+trace+" "+runCycles+" >>"+RWOutputs+config.split('.')[0]+"_"+trace
                os.system(cmd)
