...
OutDir='/home/febin/Linux2/OpticalMemoryTrace/NVMainRuns/RWOutputs/'
...
MemoryCOnfigs= ['3D_DRAM', '2D_DRAM', 'PCM_ISSCC', 'COSMOS_EPCM', 'COSMOS_OPCM', 'COSMOS_OPCM_TE']
...
for Output in sorted(os.listdir(OutDir)):
    ...
    if '3D_DRAM' in Output:
        CycTime = 1/(1.3e9)
        FilePath = DestDir+ '3D_DRAM'
    elif '2D_DRAM' in Output:
        CycTime = 1/(666e6)
        FilePath = DestDir+ '2D_DRAM'
    elif 'PCM_ISSCC' in Output:
        CycTime = 1/(400e6)
        FilePath = DestDir+ 'A_PCM_ISSCC'
    elif 'COSMOS_EPCM' in Output:
        CycTime = 1/(1e9)
        FilePath = DestDir+ 'COSMOS_EPCM'
    elif 'COSMOS_OPCM' in Output:
        if 'COSMOS_OPCM_TE' in Output:
            CycTime = 1/(1e9)
            FilePath = DestDir+ 'COSMOS_OPCM_TE'
        else:
            CycTime = 1/(1e9)
            FilePath = DestDir+ 'COSMOS_OPCM'
